...
def test_row_count(data: pd.DataFrame) -> None:
    """
    Test that the dataset has a reasonable number of rows: not too few, not too many.
...
def test_price_range(data: pd.DataFrame, min_price: float, max_price: float) -> None:
    """
    Test that all prices in the dataset fall within the expected min/max range.
